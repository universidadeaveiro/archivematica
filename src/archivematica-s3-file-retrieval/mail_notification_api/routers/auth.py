...
# @Last Modified time: 28-05-2022 16:45:41
# @Description: 
...
def login_for_access_token(form_data: schemas.UserLogin, 
...
def register_new_user(new_user: schemas.UserRegister, 
...
def update_password(password_data: schemas.NewPassword, 
    token: str = Depends(auth.oauth2_scheme), db: Session = Depends(get_db)):
    try:
        username = auth.get_current_user(token)
