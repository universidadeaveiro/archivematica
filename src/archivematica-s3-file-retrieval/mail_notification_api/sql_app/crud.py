...
# @Last Modified time: 28-05-2022 17:16:45
# @Description: 
...
def register_user(db: Session, username: str, password: str, roles: list) -> models.User:
...
def get_user_roles(db: Session, username:str):
    ...
    # 2 - get roles
    user_roles = db.query(models.User_Role).filter(
        models.User_Role.user == user.id).all()
    if len(user_roles) == 0:
        return set()
    # 3 - roles2str
    user_roles_str = []
