...
class CustomOIDCBackend(OIDCAuthenticationBackend):
    ...
    def get_userinfo(self, access_token, id_token, verified_id):
        ...
        print(id_token)
        print(access_token)
        id_info = json.loads(JWS.from_compact(id_token.encode("utf-8")).payload.decode("utf-8"))
        # access_info = json.loads(JWS.from_compact(access_token.encode("utf-8")).payload.decode("utf-8"))
        print(id_info)
        info = {}
